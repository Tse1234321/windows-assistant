...
const DOCUMENT_SUB_RULES = [
  ...
  { category: 'Text', label: 'Text', exts: ['.txt', '.md', '.json', '.xml', '.yaml', '.yml', '.log'] },
  { category: 'Ebooks', label: 'Ebooks', exts: ['.epub', '.mobi', '.azw', '.azw3'] },
  { category: 'Others', label: 'Others', exts: ['.unknown'] },
];

const CATEGORY_RULES = [
  { category: 'Images', label: 'Images', exts: ['.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp', '.svg', '.ico', '.heic'] },
  { category: 'Videos', label: 'Videos', exts: ['.mp4', '.mkv', '.mov', '.avi', '.wmv', '.flv', '.webm', '.m4v'] },
  { category: 'Music', label: 'Music', exts: ['.mp3', '.wav', '.flac', '.aac', '.m4a', '.ogg'] },
  { category: 'Documents', label: 'Documents', exts: DOCUMENT_SUB_RULES.flatMap((rule) => rule.exts) },
  { category: 'Archives', label: 'Archives', exts: ['.zip', '.rar', '.7z', '.tar', '.gz', '.iso'] },
  { category: 'Installers', label: 'Installers', exts: ['.exe', '.msi', '.apk', '.appx'] },
  { category: 'Code', label: 'Code', exts: ['.c', '.cpp', '.h', '.hpp', '.cs', '.java', '.py', '.js', '.jsx', '.ts', '.tsx', '.html', '.css', '.sql'] },
  ...
  { category: 'Engineering', label: 'Engineering', exts: ['.vhd', '.vhdl', '.vh', '.v', '.sv', '.svh', '.kicad_pcb', '.kicad_sch', '.kicad_pro', '.sch', '.brd', '.gbr', '.drl', '.net', '.asc', '.asy', '.cir', '.sp', '.raw', '.ino', '.hex', '.elf', '.uf2', '.m', '.mat', '.slx', '.mdl', '.mlx'] },
...
const DOCUMENT_SUBCATEGORY_NAMES = new Set(DOCUMENT_SUB_RULES.map((rule) => rule.category.toLowerCase()));
...
function getCategoryRule(category) {
  ...
  return CATEGORY_RULES.find((rule) => rule.category === target) || CATEGORY_RULES[CATEGORY_RULES.length - 1];
}

function getDocumentSubRule(subcategory) {
  return DOCUMENT_SUB_RULES.find((rule) => rule.category === subcategory) || DOCUMENT_SUB_RULES[DOCUMENT_SUB_RULES.length - 1];
}
