...
const BUILTIN = [
  ...
  { category: 'Git / GitHub', cmd: 'git commit -am "訊息"', desc: '已追蹤檔案一步完成 add + commit' },
  { category: 'Git / GitHub', cmd: 'git push', desc: '把本機 commit 推送到遠端（GitHub）' },
  { category: 'Git / GitHub', cmd: 'git push -u origin <branch>', desc: '首次推送並建立分支追蹤，之後可直接 git push' },
  ...
  { category: 'Git / GitHub', cmd: 'git switch -c <name>', desc: '建立並切換到新分支（等同 git checkout -b）' },
  { category: 'Git / GitHub', cmd: 'git switch <branch>', desc: '切換到既有分支' },
  { category: 'Git / GitHub', cmd: 'git merge <branch>', desc: '把指定分支合併進目前分支' },
  { category: 'Git / GitHub', cmd: 'git rebase <branch>', desc: '把目前分支的提交移到指定分支之上（線性歷史）' },
  { category: 'Git / GitHub', cmd: 'git stash', desc: '暫存目前未提交的變更，讓工作區乾淨' },
  { category: 'Git / GitHub', cmd: 'git stash pop', desc: '取回最近一次 stash 的變更' },
  { category: 'Git / GitHub', cmd: 'git reset --soft HEAD~1', desc: '撤銷最後一次 commit，但保留變更在暫存區' },
  { category: 'Git / GitHub', cmd: 'git reset --hard HEAD~1', desc: '撤銷最後一次 commit 並丟棄變更（危險，不可復原）' },
  { category: 'Git / GitHub', cmd: 'git revert <commit>', desc: '用一個新 commit 來反轉某次提交（安全、保留歷史）' },
  { category: 'Git / GitHub', cmd: 'git restore <file>', desc: '把檔案還原成上次提交的內容（丟棄未存變更）' },
  { category: 'Git / GitHub', cmd: 'git log --oneline --graph --all', desc: '用簡潔圖形方式檢視提交歷史與分支' },
  { category: 'Git / GitHub', cmd: 'git diff', desc: '查看尚未暫存的具體差異' },
  { category: 'Git / GitHub', cmd: 'git remote -v', desc: '查看目前設定的遠端網址' },
  { category: 'Git / GitHub', cmd: 'git remote add origin <url>', desc: '為本機倉庫新增名為 origin 的遠端' },
  { category: 'Git / GitHub', cmd: 'git push --force-with-lease', desc: '安全的強制推送（rebase 後常用，不會蓋掉別人的提交）' },
  { category: 'Git / GitHub', cmd: 'git tag v1.0.0 && git push --tags', desc: '建立版本標籤並推送到遠端' },
  { category: 'Git / GitHub', cmd: 'gh repo create <name> --public --source=. --push', desc: '用 GitHub CLI 直接建立遠端倉庫並推送目前資料夾' },
  { category: 'Git / GitHub', cmd: 'gh pr create', desc: '用 GitHub CLI 從目前分支建立 Pull Request' },
  ...
  { category: 'npm / Node', cmd: 'npm install -D <pkg>', desc: '安裝為開發相依（devDependencies）' },
  { category: 'npm / Node', cmd: 'npm install -g <pkg>', desc: '全域安裝（提供 CLI 指令用）' },
  { category: 'npm / Node', cmd: 'npm run dev', desc: '執行 package.json scripts 裡的 dev（開發伺服器）' },
  ...
  { category: 'Python', cmd: 'venv\\Scripts\\activate', desc: '啟用虛擬環境（Windows PowerShell/CMD）' },
  ...
  { category: 'Python', cmd: 'python -m <module>', desc: '以模組方式執行（例如 python -m http.server）' },

  // ---- 編譯：通用語言 ----
  { category: '編譯指令', cmd: 'gcc main.c -o main', desc: 'C：編譯成執行檔 main' },
  { category: '編譯指令', cmd: 'g++ main.cpp -o main', desc: 'C++：編譯成執行檔 main' },
  { category: '編譯指令', cmd: 'cmake -B build && cmake --build build', desc: 'C/C++（CMake）：設定並建置到 build 資料夾' },
  ...
  // ---- 編譯：電機／嵌入式（本機已安裝工具鏈）----
  { category: '編譯指令', cmd: 'arduino-cli compile --fqbn arduino:avr:uno sketch', desc: 'Arduino：編譯 sketch 資料夾（板型 Uno）' },
  { category: '編譯指令', cmd: 'arduino-cli upload -p COM3 --fqbn arduino:avr:uno sketch', desc: 'Arduino：上傳到開發板（COM3 改成你的埠）' },
  { category: '編譯指令', cmd: 'iverilog -o sim.out top.v tb_top.v && vvp sim.out', desc: 'Verilog：用 Icarus 編譯並執行模擬' },
  { category: '編譯指令', cmd: 'ghdl -a top.vhd && ghdl -e top && ghdl -r top', desc: 'VHDL：GHDL 分析→建構→執行模擬' },
  { category: '編譯指令', cmd: 'arm-none-eabi-gcc -c -mcpu=cortex-m4 -mthumb main.c -o main.o', desc: 'STM32：用 ARM GCC 把 C 編譯成目標檔' },
  { category: '編譯指令', cmd: 'octave --no-gui script.m', desc: 'MATLAB/Octave：以無視窗模式執行 .m 腳本' },
];
...
export default function CommandCheatsheet() {
  ...
  const all = useMemo(
    () => [
      ...BUILTIN,
      ...custom.map((c, i) => ({ category: c.category || CUSTOM_CATEGORY, cmd: c.cmd, desc: c.desc, custom: true, idx: i })),
    ],
    [custom],
  );

  const categories = useMemo(() => ['全部', ...Array.from(new Set(all.map((x) => x.category)))], [all]);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<StatusBadge tone="muted">{totalShown} / {all.length} 條</StatusBadge>}
      />
      ...
      <Card title="新增我的指令" icon="＋" style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <input
            style={{ ...inputStyle, flex: '2 1 280px', fontFamily: '"Cascadia Code","Consolas",monospace' }}
            value={form.cmd}
          ...
          <input
            ...
            onKeyDown={(e) => { if (e.key === 'Enter') addCustom(); }}
          />
          <Button variant="primary" onClick={addCustom}>新增</Button>
        </div>
      </Card>

      {grouped.length === 0 ? (
        <Card>
          <EmptyState title="找不到符合的指令" description="換個關鍵字，或把分類切回「全部」再試一次。" />
        </Card>
      ) : (
              ...
              {items.map((item, index) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {item.desc ? (
                      <div className="muted" style={{ fontSize: 12, marginTop: 4 }}>{item.desc}</div>
                    ) : null}
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    <Button size="sm" variant="ghost" onClick={() => copy(item.cmd)}>複製</Button>
                    {item.custom ? (
                      <Button size="sm" variant="danger" onClick={() => removeCustom(item.idx)}>刪除</Button>
                    ) : null}
                  </div>
                </div>
              ))}
