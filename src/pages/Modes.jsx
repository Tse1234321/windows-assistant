...
function modeHaystack(mode) {
  return [
    mode.name,
    ...(mode.apps || []).map((item) => (typeof item === 'string' ? item : `${item.name || ''} ${item.path || ''} ${item.workspaceFolder || ''}`)),
    ...(mode.folders || []),
    ...(mode.urls || []),
...
function modeSize(mode) {
  return (mode.apps?.length || 0) + (mode.folders?.length || 0) + (mode.urls?.length || 0) + (mode.commands?.length || 0);
}
      ...
      <PageHeader
        ...
        actions={(
          <>
            ...
            <Button variant={editing ? 'primary' : 'ghost'} onClick={() => setEditing((value) => !value)}>
              {editing ? '完成編輯' : '編輯模式'}
            </Button>
          </>
        )}
      ...
      {modes.length === 0 ? (
        <Card>
          <EmptyState
            ...
            action={<Button variant="primary" onClick={() => setEditing(true)}>建立第一個模式</Button>}
          />
        </Card>
      ) : filteredModes.length === 0 ? (
        <Card>
          <EmptyState title="找不到符合的工作模式" description="換個關鍵字或清除篩選條件後再試一次。" />
        </Card>
      ) : (
          ...
          {filteredModes.map((mode) => (
            <Card key={mode.name} className="mode-card">
              <div className="mode-card-head">
                ...
                <Button variant="primary" busy={busyMode === mode.name} onClick={() => run(mode.name)}>
                  啟動
                </Button>
              </div>
      ...
      {result ? (
        <Card title={`執行結果：${result.mode || '--'}`} icon="RS" style={{ marginTop: 16 }}>
          <div className="result-strip">
            <StatusBadge tone={result.ok ? 'ok' : 'warn'}>{result.ok ? '完成' : '部分失敗'}</StatusBadge>
          ...
          {resultRows.length > 0 ? (
            <DataTable
              rows={resultRows}
              columns={[
                { key: 'type', label: '類型', render: (row) => <span className="tag">{STEP_LABEL[row.type] || row.type}</span> },
                { key: 'target', label: '目標', render: (row) => <span className="path">{row.target}</span> },
                {
                  key: 'status',
                  label: '狀態',
                  render: (row) => (
                    <span className={row.status === 'ok' ? 'status-ok' : row.status === 'skipped' ? 'muted' : 'status-error'}>
                      {row.status === 'ok' ? '成功' : row.status === 'skipped' ? '略過' : '失敗'}
                    </span>
                  ),
                },
                { key: 'message', label: '訊息', render: (row) => <span className="muted">{row.message}</span> },
              ]}
            />
          ) : null}
        </Card>
