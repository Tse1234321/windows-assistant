...
export default function ActivityHistory() {
  ...
  const restoreDownloads = async () => {
    ...
    setBusy(false);
    toast(result.ok ? `已復原 ${result.restored || 0} 個項目` : result.error || '復原失敗', result.ok ? 'ok' : 'error');
    load();
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button variant="primary" busy={busy} onClick={restoreDownloads}>復原上次 Downloads 整理</Button>}
      />

      <InlineAlert tone="info" title="復原範圍">
        目前可復原 Downloads 的移動整理紀錄；Clean Center 是移到資源回收筒，請在資源回收筒清空前還原。
      </InlineAlert>
      ...
      <SectionPanel title="活動歷史" description="依時間排序顯示最近 250 筆事件。">
        <DataTable
          ...
          columns={[
            ...
            { key: 'type', label: '類型', render: (row) => <StatusBadge tone={typeTone(row.type)}>{row.type}</StatusBadge> },
