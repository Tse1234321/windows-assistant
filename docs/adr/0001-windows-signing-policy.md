...
- The tag release workflow uses `release:signed` and receives credential values only from GitHub Actions secrets.
...
- A public tag workflow now fails clearly until trusted signing secrets are configured.
